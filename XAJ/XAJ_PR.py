...
class XAJ_PR():
    ...
    def judge_flood_type(self):
        ...
        '''
            dongpu:
                大洪水：以起始时刻之前的第23个时刻为起点，用户输入的降雨时段终止时刻为终点，该段时间内任意连续24小时降水量之和大于100mm，或者该时段内至少有一个时段降水量大于45mm。
                小洪水：以起始时刻之前的第23个时刻为起点，用户输入的降雨时段终止时刻为终点，该段时间内任意连续24小时降水量之和大于26mm且小于100。
                非洪水：以起始时刻之前的第23个时刻为起点，用户输入的降雨时段终止时刻为终点，该段时间内任意连续24小时降水量之和小于26mm。
            dafangying:
                20 以上为 洪水， 20以下为非洪水。
        '''
        start = 24
        end = len(self.data['rainfull']) - 72

        flag = 0
        # 滑动窗口计算任意连续24消失的降雨量之和
        for i in range(start, end):
            if self.type == "0":
                if sum(self.data['rainfull'][i-24:i]) >= 100 or max(self.data['rainfull'][i-24:i]) >= 45:
                    flag = max(flag, 2)
                elif sum(self.data['rainfull'][i-24:i]) >= 26 and sum(self.data['rainfull'][i-24:i]) < 100:
                    flag = max(flag, 1)
                else:
                    flag = max(flag, 0)
            else:
                if sum(self.data['rainfull'][i-24:i]) >= 20:
                    flag = max(flag, 1)
                else:
                    flag = max(flag, 0)
    ...
    def load_data_from_monitor_db(self):
        ...
        try:
            ...
            sql = "select * from {} where TM between '{}' and '{}' order by TM".format(
                PPTN_table_name, start_time, end_time)
            ...
            # 根据start_time和end_time计算长度(小时)
            start_time = datetime.datetime.strptime(
                start_time, "%Y-%m-%d %H:%M:%S")
            # start_time向前推23个小时
            start_time = start_time - datetime.timedelta(hours=23)
            ...
            # 获取前20天的面雨量数据
            pa_start_time = datetime.datetime.strptime(self.ins.start, "%Y-%m-%d %H:%M:%S") - datetime.timedelta(days=22)
            pa_end_time = datetime.datetime.strptime(self.ins.start, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(hours=47)
            ...
            if len(tmp) == 0:
                surface_pptn_day = [0] * 20

            day_rainfall = 0

            index = 0
            for row in tmp:
                surface_pptn_hour.append(float(row[3]))
                # print(row[1].hour, day_rainfall, row[3])
                if row[1].hour == 9:
            ...
            # 若最后一个时刻不为9点，则去掉向前到九点的数据
            if tmp[-1][1].hour != 9:
                if tmp[-1][1].hour > 9:
                    remove_index = tmp[-1][1].hour - 9
                else:
                    remove_index = tmp[-1][1].hour + 24 - 9
                
                print("remove index: ", remove_index)
                
                surface_pptn_hour = surface_pptn_hour[:-remove_index]
            ...
            # 若不足480个，前面补0
            if len(surface_pptn_hour) < 480:
                surface_pptn_hour = [0] * (480 - len(surface_pptn_hour)) + surface_pptn_hour
            
            self.data["day_rainfall"] = surface_pptn_day
            # self.data["hour_rainfall"] = surface_pptn_hour
            ...
            logging.error("load data from db error, error: {}".format(e))
            print(e)
    ...
    def load_data_from_db(self):
        ...
        try:
            ...
            print(start_time, end_time, _batch, _type)
            ...
            sql = "SELECT (INFLOW) FROM {} WHERE TM BETWEEN '{}' and '{}' and BATCH = '{}' and TYPE = '{}' order by TM".format(
                in_outflow_table_name, start_time, end_time, _batch, _type)
            cursor.execute(sql)
            ...
            # 获取数据
            sql = "select (value) from {} where TIME between '{}' and '{}' and BATCH = '{}' and TYPE = '{}' order by TIME".format(
                rainfall_table_name, start_time, end_time, _batch, _type)
            cursor.execute(sql)
            ...
            # 读取蒸发数据
            sql = "select (value) from {} where TIME between '{}' and '{}' and BATCH = '{}' and TYPE = '{}' order by TIME".format(
                evaporation_table_name, start_time, end_time, _batch, _type)
            cursor.execute(sql)
            ...
            # 读取实测数据  从start_time 到 end_time之间的数据，若有则取，若无则补-1
            sql = "select * from {} where TIME between '{}' and '{}' and TYPE = '{}' and BATCH = '{}' order by TIME".format(
                inflow_table_name, start_time, end_time, _type, "10000")
            cursor.execute(sql)
            ...
            length = int((datetime.datetime.strptime(
                end_time, "%Y-%m-%d %H:%M:%S") - datetime.datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")).total_seconds() / 60 / 60) + 1 - 72
            ...
            for i in range(length):
                timex = datetime.datetime.strptime(
                    start_time, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(hours=i)
                timex = str(timex.strftime("%Y-%m-%d %H:%M:%S"))
            ...
            # 若无蒸发量数据，补为0
            if len(self.data['evaporation']) == 0:
                self.data['evaporation'] = [0]

            # 若将蒸发量补全到和降雨量一样的长度
            if len(self.data['evaporation']) < len(self.data['rainfull']):
                self.data['evaporation'] += [self.data['evaporation'][-1]] * \
                    (len(self.data['rainfull']) -
                     len(self.data['evaporation']))
            # 数据长度
            ...
            if len(self.data['inflow_x']) < len(self.data['rainfull']):
                self.data['inflow_x'] += [self.data['inflow_x'][-1]] * \
                    (len(self.data['rainfull']) - len(self.data['inflow_x']))
            if len(self.data['outflow_x']) < len(self.data['rainfull']):
                self.data['outflow_x'] += [self.data['outflow_x'][-1]] * \
                    (len(self.data['rainfull']) - len(self.data['outflow_x']))
    ...
    def write_data_to_db(self):
        ...
        try:
            ...
            for i in range(len(self.result_xaj)):
                ...
                # sql = "INSERT INTO {} (TIME, BATCH, QX, QP, PR_R, TYPE) VALUES ('{}', '{}', {}, {}, {}, {}) ON DUPLICATE KEY UPDATE QX = {}, QP = {}, PR_R = {}".format(table_name, timex, batch, QX, QP, PR_R, _type, QX, QP, PR_R)
            ...
            print(e)
            logging.error("write data to db error, error: {}".format(e))
            exit(0)
    ...
    def PR_rainstorm_center(self):
        ...
        point_rainfall_copy = self.data['point_rainfall']

        for station in upstream_station:
            print("STATION: ", station , point_rainfall_copy[station][47+23:-72])

            station_rainfall_sum[station] = sum(
                point_rainfall_copy[station][47+23:-72])
            
        for station in downstream_station:
            station_rainfall_sum[station] = sum(
                point_rainfall_copy[station][47+23:-72])
            
        print(station_rainfall_sum)
    ...
    def PR_calculate(self):
        ...
        rainfall_copy = self.data['rainfull'].copy()
        for t in range(47):
            rainfall_copy[t] = 0

        rainstorm_center = self.PR_rainstorm_center()

        print("tainstorm_center:", rainstorm_center)

        print("Pa: ", Pa)
        print("K: ", K)
        
        for t in range(N):          # PR模型前47个点不计算
            P = float(rainfall_copy[t])
            SUM_P += P
            ...
            if rainstorm_center == 1:
                ...
                if self.ins.type == "0":
                    R[t] = 0.00174276402839024 * (SUM_P + Pa) ** 1.92834705920637
                else:
        ...
        for i in range(len(Q)):
            ...
            time_index = datetime.datetime.strptime(
                start_time, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(minutes=60*i)
    ...
    def correct_water_module(self, Q):
        '''
            修正规则：
                从-47时刻开始，找连续48小时降雨之和大于等于5的时刻，以该时刻为分界线进行修正，
                1）该时刻之前的流量修正方法为：各时刻流量值为预报值减去14.486（改为：减去15），减后数值若小于0则赋值为0。
                2）
                    1 该时刻起（含该时刻）的流量修正方法为：预报流量过程中某一时刻若大于等于30，不修正，
                    2 若小于30，且下时刻降雨为0，则下时刻流量值为上时刻预报流量*0.97），
                    3 预报流量某一时刻流量若小于17，且下时刻降雨为0，则下时刻流量值为上时刻预报流量*0.92，
                    4 流量若小于6.，且下时刻降雨为0，则下时刻流量为上时刻预报流量*0.85；
                    5 若小于2，且下时刻降雨为0，则下时刻流量值为上时刻预报流量*0.8
                    6 小于0.6，且下时刻降雨为0，则下时刻流量值为0。
        '''
        # 针对连续48小时不降雨进行修正
        # 找到第一个不为0的流量值所在的位置
        n1 = 48
        ...
        for i in range(0, n1 - 1):
            sum_rainfall_tmp += self.data["rainfull"][i]
            if sum_rainfall_tmp >= 5:
                start_index = i
                break
        
        if sum_rainfall_tmp < 5:
            for i in range(n1, len(Q)):
                if sum(self.data["rainfull"][i - n1: i]) >= 5:
                    start_index = i
                    break
    ...
    def cal_water_z(self, Q, inflow, outflow):
        ...
        # for i in range(len(Q)):
        #     print("Q: ", Q[i], "inflow: ", inflow[i], "outflow: ", outflow[i])
